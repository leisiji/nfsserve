...
use nfsserve::nfs::*;
use nfsserve::udp::{NFSUdp, NFSUdpListener};
use nfsserve::vfs::{DirEntry, NFSFileSystem, ReadDirResult, VFSCapabilities};
use tokio::fs::{File, OpenOptions};
use tokio::io::{AsyncReadExt, AsyncSeekExt, AsyncWriteExt};
use tracing::debug;

/// Cross-platform conversion from NFS byte strings to OsString.
/// NFSv3 filenames are opaque byte strings. On Unix they map to OS bytes
/// directly; on Windows we go through String::from_utf8_lossy to handle
/// any non-UTF-8 sequences gracefully.
...
impl FSMap {
    fn new(root: PathBuf) -> FSMap {
        ...
        let root_entry = FSEntry {
            name: Vec::new(),
            fsmeta: metadata_to_fattr3(0, &root.metadata().unwrap()),
            children_meta: metadata_to_fattr3(0, &root.metadata().unwrap()),
            children: None,
        };
        FSMap {
            root,
    ...
    async fn find_child(&self, id: fileid3, filename: &[u8]) -> Result<fileid3, nfsstat3> {
        ...
        name.push(
            self.intern
                .check_interned(&bytes_to_os_string(filename))
    ...
    async fn refresh_dir_list(&mut self, id: fileid3) -> Result<(), nfsstat3> {
        ...
        if let Ok(mut listing) = tokio::fs::read_dir(&path).await {
            ...
            // Sync children_meta with current fsmeta to avoid re-listing on every readdir
            if let Some(ent) = self.id_to_path.get_mut(&id) {
                ent.children_meta = ent.fsmeta;
            }
        }
...
impl MirrorFS {
    ...
    async fn create_fs_object(
        ...
        let objectname_osstr = bytes_to_os_string(objectname);
        ...
        match object {
            ...
            CreateFSObject::Symlink((_, target)) => {
                ...
                std::os::windows::fs::symlink_file(bytes_to_os_string(target), &path)
...
impl NFSFileSystem for MirrorFS {
    ...
    async fn lookup(&self, dirid: fileid3, filename: &filename3) -> Result<fileid3, nfsstat3> {
        ...
        let objectname_osstr = bytes_to_os_string(filename);
        path.push(&objectname_osstr);
        if !exists_no_traverse(&path) {
    ...
    async fn remove(&self, dirid: fileid3, filename: &filename3) -> Result<(), nfsstat3> {
        ...
        let ent = fsmap.find_entry(dirid)?;
        let mut path = fsmap.sym_to_path(&ent.name).await;
        path.push(bytes_to_os_string(filename));
        if let Ok(meta) = path.symlink_metadata() {
            ...
            let filesym = fsmap.intern.intern(bytes_to_os_string(filename)).unwrap();
    ...
    async fn rename(
        ...
        from_path.push(bytes_to_os_string(from_filename));
        ...
        to_path.push(bytes_to_os_string(to_filename));
        ...
        let oldsym = fsmap.intern.intern(bytes_to_os_string(from_filename)).unwrap();
        let newsym = fsmap.intern.intern(bytes_to_os_string(to_filename)).unwrap();
    ...
    async fn readlink(&self, id: fileid3) -> Result<nfspath3, nfsstat3> {
        ...
        if path.is_symlink() {
            if let Ok(target) = path.read_link() {
                Ok(target.as_os_str().as_encoded_bytes().into())
            } else {
