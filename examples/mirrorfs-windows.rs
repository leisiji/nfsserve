...
pub fn metadata_to_fattr3(fid: fileid3, meta: &Metadata) -> fattr3 {
    ...
    fattr3 {
        ...
        fileid: fid,
        atime: nfstime3 {
            seconds: accessed.as_secs() as u32,
            nseconds: accessed.as_nanos() as u32,
        },
        mtime: nfstime3 {
            seconds: modified.as_secs() as u32,
            nseconds: modified.as_nanos() as u32,
        },
        ctime: nfstime3 {
            seconds: created.as_secs() as u32,
            nseconds: created.as_nanos() as u32,
        },
    }
}
...
impl FSMap {
    ...
    async fn find_child(&self, id: fileid3, filename: &[u8]) -> Result<fileid3, nfsstat3> {
        ...
        name.push(
            self.intern
                .check_interned(from_bytes(&nfsstring::from(filename)).as_os_str())
                .ok_or(nfsstat3::NFS3ERR_NOENT)?,
        );
...
fn from_bytes(objectname: &filename3) -> OsString {
    let objectname_osstr: Vec<u16> = objectname.iter().map(|&b| b as u16).collect();
    OsString::from_wide(&objectname_osstr)
}
...
impl MirrorFS {
    ...
    async fn create_fs_object(
        ...
        let objectname_osstr = from_bytes(objectname).to_os_string();
        ...
        match object {
            ...
            CreateFSObject::Symlink((_, target)) => {
                ...
                // Fallback to creating a file with the target path stored
                let mut file = std::fs::File::create(&path).map_err(|_| nfsstat3::NFS3ERR_IO)?;
                file.write_all(target).map_err(|_| nfsstat3::NFS3ERR_IO)?;
            }
        }
...
impl NFSFileSystem for MirrorFS {
    ...
    async fn lookup(&self, dirid: fileid3, filename: &filename3) -> Result<fileid3, nfsstat3> {
        ...
        let objectname_osstr = from_bytes(filename).to_os_string();
        path.push(&objectname_osstr);
        if !exists_no_traverse(&path) {
    ...
    async fn remove(&self, dirid: fileid3, filename: &filename3) -> Result<(), nfsstat3> {
        ...
        if let Ok(meta) = path.symlink_metadata() {
            ...
            let filesym = fsmap
                .intern
                .intern(from_bytes(filename).to_os_string())
                .unwrap();
    ...
    async fn rename(
        ...
        from_path.push(from_bytes(from_filename));
        ...
        to_path.push(from_bytes(to_filename));
        ...
        let oldsym = fsmap
            .intern
            .intern(from_bytes(from_filename).to_os_string())
            .unwrap();
        let newsym = fsmap
            .intern
            .intern(from_bytes(to_filename).to_os_string())
            .unwrap();
